...
class Autoencoder(PerceptronMulticapa):
    ...
    def fit(self, X_train, y_train, learn_rate=1, epoch=100, error=-1,
            normalizar=None):
        ...
        for epoca in range(1, epoch + 1):
            ...
            for x, y in zip(X_train, y_train):
                ...
                pixeles_errados = np.abs(((y > 0).astype(int) - (self.fyin[-1] > 0).astype(int))).sum()
                pe[epoca-1] += pixeles_errados

                if pixeles_errados == 0:
                    lrc[epoca - 1] += 1
